...
export default async function PractitionerDashboard() {
  ...
  const user = db.prepare('SELECT email FROM users WHERE id = ? AND role = ?')
    .get(userId, 'practitioner') as { email: string } | undefined;
  ...
  return (
    <>
      <Navbar userEmail={user.email} userRole="practitioner" />
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-[#1E293B] rounded-xl p-8">
            ...
            <p className="text-[#94A3B8]">
              Hello, {user.email}! You are logged in as a practitioner.
            </p>
          </div>
        </div>
      </div>
